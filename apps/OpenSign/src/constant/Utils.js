...
export const getAppLogo = async () => {
  ...
  try {
    ...
    if (tenant) {
      ...
      localStorage.setItem("appname", "Justice On Call™");
...
export const getTenantDetails = async (objectId, contactId) => {
  try {
    ...
    console.error("err in gettenant", err?.response?.data || err);
    return "";
  }
};
...
export const mailTemplate = (param) => {
  const appName = "Justice On Call™";
  const logo = `<div style="padding:20px;text-align:center;"><img src="https://sign.justiceoncall.ca/assets/justice-document-sign-logo.png" alt="Justice On Call™" height="60" style="display:inline-block;height:60px;width:auto;" /></div>`;
