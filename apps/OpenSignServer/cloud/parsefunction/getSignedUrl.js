...
export function getSignedLocalUrl(fileUrl, expirationTimeInSeconds) {
  const publicBase = (process.env.SERVER_URL || '').replace(/\/app\/?$/, '/api');
  if (publicBase && fileUrl?.startsWith(process.env.INTERNAL_SERVER_URL || '___NO_INTERNAL___')) {
    fileUrl = publicBase + fileUrl.slice(process.env.INTERNAL_SERVER_URL.length);
  }
...
export function presignedlocalUrl(signedUrl, expirationTimeInSeconds) {
  if (signedUrl?.includes('/files/')) {
    ...
    const internalBase = process.env.INTERNAL_SERVER_URL || '';
    const publicBase = (process.env.SERVER_URL || '').replace(/\/app\/?$/, '/api');
    if (internalBase && publicBase && fileUrl.startsWith(internalBase)) {
      fileUrl = publicBase + fileUrl.slice(internalBase.length);
    }
    const secretKey = process.env.MASTER_KEY;
    const exp = expirationTimeInSeconds || 200;
    try {
